...
WebUI.click(findTestObject('Object Repository/Page_Tokobuah Admin - Products/th_Price'))

WebUI.click(findTestObject('Object Repository/Page_Tokobuah Admin - Products/td_Salak'))

String expectedValue='Salak'

WebDriver driver = DriverFactory.getWebDriver()

WebElement table = driver.findElement(By.xpath('//table/body'))
...
table: for(int i = 0; i<Rows.size(); i++){
	List<WebEl> Cols = Rows.get(i).findElements(By.tagName('td'))
	for(int j = 0; j<Cols.size(); j++){
		if(Cols.get(j).getText().equalsIgnoreCase(expectedValue)){
			WebUI.commnet('Kata' + expectedValue+ 'ditemukan')
			break
		}
	}
}
